...
build_direct() {
    ...
    for program in \
        ...
        experiment8_interpolation_endpoint; do
        "$cxx" $common -DTGI_RESULTS_DIR=\"results\" \
            "experiments/$program.cpp" -o "$build_dir/$program"
    done
}
