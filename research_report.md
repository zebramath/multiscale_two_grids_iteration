...
脚本优先使用 CMake，并提供等价的严格 C++17 直接构建路径。六份完整逐行结果位于
`code/results`；`supplemental` 只重现实验 5--6，`full` 重现全部实验，`quick` 用于
开发检查并默认写入 `code/build/quick-results`，不会覆盖正式结果。v6.3 交付中实验
1--4 保留已验证的 v6.2 完整结果，新增实验 5--6 由 6.3.0 源码运行生成。
