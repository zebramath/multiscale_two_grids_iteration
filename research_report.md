...
七个实验程序保留独立输出，正文按四个数值主题组织：

| 主题 | 对应实验 | 核心问题 |
|---|---|---|
| 两网格性能与有限路径机制 | Exp1--2 | 跨尺度、对比度和拓扑的主结果，以及能量下降与两网格性能的非单调关系 |
| 自适应规则的选择质量、稳健性与代价 | Exp3--5 | 离线采样参考差距、seed/RHS 迁移、重复计时和停止策略消融 |
| 固定物理系数场加密 | Exp6 | 同一连续系数函数下的三层嵌套加密表现 |
| 多层初步验证 | Exp7 | 三层 V-cycle 的有限层次可行性和递归代价 |
...
## 7. 自适应规则的选择质量、稳健性与代价（Exp3--5）

### 7.1 窗口受限离线采样参考

离线采样参考（sampled oracle）检查 $m=0$ 以及从 $n/8$ 到 $n/2$ 的 step-2 路径点。
七个设计问题用于设置归一化区间，三个参数冻结后的新组合用于独立检查。该参考定义为
...
|---|---:|---:|---:|---:|
| adaptive | 459.730（436.142--471.128） | 287.550（285.798--299.807） | 753.910（724.335--770.350） | 242 |
| global-reference | 2582.018（2560.104--2646.048） | 9591.914（9363.578--10510.346） | 12152.018（12009.626--13092.364） | 3227 |

adaptive 在中心问题选择 $m=43$；该环境中 global-reference 的 setup、循环数和完整
求解时间均更高，total 中位数之比为 16.12。墙钟时间依赖硬件、编译器和系统负载，
...
使用完全相同的首层插值算子运行精确两网格。表中插值复杂度为
$C_P=\sum_{\ell=0}^{L-1}\operatorname{nnz}(P_\ell)/\operatorname{nnz}(A_0)$。

| 案例/层次 | adaptive：V/TG 循环 | global-reference：V/TG 循环 | $C_P$：adaptive/reference | geometric V-cycle |
|---|---:|---:|---:|---:|
| cross, 64/16/8 | 101/65 | 321/321 | 12.5962/42.4913 | 30000，slow-limit |
| ring, 128/16/8 | 237/237 | 770/768 | 13.4528/44.1727 | 30000，slow-limit |

adaptive 与 global-reference 的四个 V-cycle 均达到 $10^{-6}$，有限步构造在两例中使用
更少循环和更低插值复杂度。该 pilot 支持三层递归的有限层次可行性；完整逐行结果及
$C_A$、收敛因子和最终残量见 Exp7 输出。
...
V-cycle 递归误差分解；实验从两网格性能与有限路径、自适应规则评估、固定物理场加密
和多层 pilot 四个主题检查这一机制。
...
目录分离。七组实验均由 v7.6.0 数值源码以完整模式重新运行；构建环境、检查项目和
运行来源见 `VALIDATION_v7.6.md`。
