...
本文采用一个理论知情的轻量经验选择器；其具体步数比例和尺度比阈值不附带最优性主张。
...
3. 从标准 PCG 理论提取 $O(h^{-1})$ 的充分步数尺度，并据此构造低 setup 的轻量经验
   单候选；
...
最大 gap 达 54.20%，与“轻量经验选择器”而非 near-oracle 方法的定位一致。
...
求解时间均更高，total 中位数之比为 14.69。墙钟时间依赖硬件、编译器和系统负载，
因此循环数与收敛因子仍作为主要可移植指标。

## 10. 停止策略消融

为区分经验选择器与一般意义上的“提前停止”，设置两个不随问题重新调参的简单
...
列残量阈值不能稳定替代同时利用网格尺度和对角尺度比的经验选择器。
...
每层均不高于另外两个比较对象。该有限序列没有显示随两次二分加密而系统恶化，构成固定
物理几何下的网格加密证据；它不被解释为无限细化下网格无关收敛的证明。
...
V/TG 比为 1.554 和 1.000，表明在这两个层次上有限步构造可递归使用。geometric 在两例
均未于上限内达到容差。该组是确定性的可行性证据，不支持对任意层数下的网格无关收敛
或最优线性复杂度作进一步断言。
...
后者显示这两个层次中 global-reference 插值的存储代价高于有限步 adaptive；该指标只描述
已构造三层层次的稀疏度，不被解释为渐近 setup 复杂度。

## 13. 结论

本项目研究一种小型而明确的算法机制：不把能量极小插值的高精度数值终点直接等同于
有限代价下最有利的两网格插值，而是在同一条预条件共轭梯度路径上选取一个
...
场加密和三层试验未显示机制立即消失。adaptive 应严格理解为
*theory-informed lightweight empirical selector*：理论支持其 $O(h^{-1})$ 尺度，具体
比例和阈值仍是经验参数。更一般的几何、离散、平滑器和并行环境仍需独立验证。
...
正式结果。七组实验均由 v7.2.0 数值源码以完整模式重新运行，结果文件不混用早期版本。
v7.3 收紧理论与方法定位；v7.4 只修订论证、术语、实验口径和输出文案，未改变数值算法、
参数或正式 TXT 数据，也未重复完整运行。当前版本的静态检查、严格构建和 quick 验证见
`VALIDATION_v7.4.md`。
