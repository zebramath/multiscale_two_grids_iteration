...
tgi::Vector normalized(tgi::Vector values) {
    ...
    if (!(length > 0.0)) {
        throw std::runtime_error("submission RHS must be nonzero");
    }
...
int run_submission_robustness(int argc, char** argv) {
    int threads = 4;
    ...
    const auto transfer_geometric =
        experiment_support::geometric_interpolation(
            grid, transfer_problem.matrix);
