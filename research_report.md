...
累计循环数分别为 4459 和 28972。cross-channel 与 winding-ring 的完整路径扫描同时
验证了插值能量的单调下降和实际有效收敛因子的非单调变化。完整理论分析单列于
`theory.tex`。
...
七个程序按四个主题组织：Exp1--2 给出主比较与两类拓扑的完整路径扫描；Exp3--5 检验
选择质量、稳健性、代价和停止策略；Exp6 考察固定物理系数场的嵌套加密；Exp7 给出
三层 V-cycle 初步结果。
...
v8.1 的数值证据支持一个集中结论：在当前高对比通道问题、固定粗点和指定磨光器下，
沿能量插值路径选择有限步位置可以同时降低构造成本和两网格循环数。两类拓扑的完整
路径扫描表明，插值能量单调下降并不要求实际有效收敛因子同步下降；主比较、seed/RHS、
停止消融和多层初试说明这一优势并非由单一算例或单一右端项产生。固定物理场加密中
优势明显缩小，因而本文最合适的数值定位是面向网格尺度通道结构的高对比求解策略，
并把固定物理加密作为边界与补充证据。
