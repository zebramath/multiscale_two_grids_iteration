...
决定。七个离线 step--2 sampled oracle 的 $m_{\mathrm{or}}/(1/h)$ 依次为 0.125、
0.125、0.281、0.297、0.203、0.484 和 0.406，均位于当前算法的 $[1/8,1/2]$
归一化窗口。这些离散采样点是尺度证据，不等同于连续候选路径上的
$m_{\mathrm{opt}}(h)$。
...
|---|---:|---:|---:|---:|
| adaptive-fast | 1230.233（945.931--1257.148） | 445.112（435.979--480.139） | 1650.996（1391.043--1765.971） | 242 |
| adaptive-reuse | 2752.070（2621.607--2950.674） | 495.515（472.339--562.377） | 3179.980（3168.931--3446.189） | 242 |
| global-reference | 4246.161（4143.377--4402.557） | 15586.291（15443.653--16661.989） | 20245.077（19988.847--20805.366） | 3227 |
...
6. 标准条件数伸缩给出固定能量衰减的 $O(h^{-1})$ 步数上界；进一步在双侧路径衰减和
   固定可见误差带下，最优有限步满足
   $c_-h^{-1}\le m_{\mathrm{opt}}\le c_+h^{-1}$；
