...
实验 1--5 的系数几何以网格单元数定义，因此这些尺寸轴表示一组受控离散问题。v6.3
另设实验 6：通道宽度、背景分区和 seed 均在连续物理坐标中固定，并同步细化细、粗
网格，从而把标准固定系数场加密与原受控离散族明确分开。
v6.4 的实验 7 在 64/16/8 与 128/16/8 层次上逐层重建 transfer，并以最粗层精确求解
构造三层 V-cycle。
...
本文不再停留于“某个方向有利”的存在性，而是求解固定能量预算下的局部最优问题。
定义
...
为区分尺度自适应规则与一般意义上的“提前停止”，新增两个不随问题重新调参的简单
...
新增固定连续系数场使用 cross-channel、对比度 $10^4$、物理通道宽度 $1/16$、固定
...
| 案例/层次 | 方法 | 层参数 | 算子复杂度 | 精确两网格 | 三层 V-cycle | V/TG | 状态 |
|---|---|---|---:|---:|---:|---:|---|
| cross, 64/16/8 | adaptive | 21/2 步 | 3.0786 | 65 | 101 | 1.554 | converged |
| cross, 64/16/8 | global-reference | $10^{-10}/10^{-10}$ | 3.7064 | 321 | 321 | 1.000 | converged |
| cross, 64/16/8 | geometric | $P_G/P_G$ | 1.1128 | 24950 | 30000 | 1.202 | slow-limit |
| ring, 128/16/8 | adaptive | 43/2 步 | 1.5379 | 237 | 237 | 1.000 | converged |
| ring, 128/16/8 | global-reference | $10^{-10}/10^{-10}$ | 1.6617 | 768 | 770 | 1.003 | converged |
| ring, 128/16/8 | geometric | $P_G/P_G$ | 1.0276 | 30000 | 30000 | 1.000 | slow-limit |
...
正式结果。v6.4 交付保留已验证的实验 1--6 结果，只有实验 7 由 6.4.0 源码新增运行。
