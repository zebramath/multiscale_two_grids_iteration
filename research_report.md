...
累计循环数分别为 4459 和 28972。中心问题的逐步收敛率扫描进一步展示有限步路径上的
...
其中 $k$ 为实际循环数，因子越小表示平均每次循环的残量压缩越强。Exp2 只展示
$\rho_{\mathrm{eff}}$ 随有限 PCG 步数的变化；其他实验将循环数与该因子配对报告。
...
Exp2 固定 128/16、对比度 $10^4$ 的 cross-channel 问题，对 $m=1,\ldots,128$ 每个整数
步数分别计算有效收敛因子。图中展示全部 128 个点；对应数值保存在
`code/results/experiment2_central_step_scan.csv`。

![中心问题逐步扫描](code/results/experiment2_central_step_scan.png)

曲线在 $m=38$ 达到全局最小值 0.9417637；adaptive 选择 $m=43$，有效收敛因子为
0.9444069；global-reference 的有效收敛因子为 0.9957271。此后曲线总体回升，同时在
$m\approx56$、$m\approx63$、$m\approx90$ 和 $m\approx108$ 附近保留局部波动。扩展到
$m=128$ 后，有限步最优区间及其与能量数值参考之间的差异仍然清晰。
...
v7.8 的数值证据支持一个集中结论：在当前高对比通道问题、固定粗点和指定磨光器下，
