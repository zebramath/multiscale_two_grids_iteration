...
| `global_pcg.hpp` | 增量全局 PCG 路径和 v4.3 单参数预算选择器 |
...
`expected_rhs_count=R` 连续控制候选间距、最大路径步数、pilot 长度和近优容忍度；
较大 `R` 还允许在预测优胜者的 `m±2` 处做至多两次细化。默认 `R=1` 只评估
`m=0,12,32,52` 且 pilot 为 16；`R=256` 评估 `m=0,12,20,...,60` 且 pilot 为
160。策略不读取尺寸、对比度、拓扑或 oracle。
