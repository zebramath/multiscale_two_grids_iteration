...
四份 TXT 均由 4.6.0 最终源码以完整模式、4 线程生成。

| 文件 | 覆盖范围 | 核心结果 |
|---|---|---|
| `experiment1_two_grid_comparison.txt` | 11 个尺寸/对比度/拓扑问题 | fast 2850 次、reuse 2382 次、reference 11074 次；fast 相对 reuse 降低 79.4% setup |
...
数值参考，而非解析精确解。所有逐问题数据均在对应 TXT 中；循环数、收敛性和选择预算
是主要证据，wall-clock 只用于策略成本方向，重复计时也只代表当前机器与固定算例。
