...
累计循环数分别为 4459 和 28972。中心问题的独立对照中，几何插值与能量极小插值分别
需要 85524 和 3227 个循环，表明能量极小插值本身已显著改善两网格收敛。cross-channel
与 winding-ring 的完整路径扫描同时验证了插值能量的单调下降和实际有效收敛因子的
非单调变化。完整理论分析单列于 `theory.tex`。
...
离散后得到对称正定系统 $A_hu_h=b_h$。系数场由随机板块背景叠加高导通道构成；通道
包括交叉、曲折、对角、平行、分支和闭合曲环六类拓扑，对比度取 $10^2$、$10^4$ 或
$10^6$。
...
两种方法采用相同粗点、对称 Gauss--Seidel 磨光和 Galerkin 粗算子。中心计时中，两者
还从同一几何插值出发并沿同一 Jacobi--PCG 路径推进。

## 2. 实验设置与指标

正式求解均从零初值开始，以相对欧氏残量 $10^{-6}$ 为收敛标准。有效收敛因子定义为
...
其中 $k$ 为实际循环数，因子越小表示平均每次循环的残量压缩越强。该经验量来自指定
右端项的残量历程，不等同于理论中的能量范数两网格因子 $\rho_{\mathrm{TG}}$。Exp2 同时
展示

$$
J(W_m)=\frac12\operatorname{tr}(P_m^\top A_hP_m)
$$

的归一化能量差与 $\rho_{\mathrm{eff}}(m)$；其他实验将循环数与 $\rho_{\mathrm{eff}}$
配对报告。Exp2 对每个路径位置最多观测 12000 个两网格循环；若未提前收敛，则以第
12000 次循环的残量计算 $\rho_{\mathrm{eff}}$。因此该曲线是统一有限观测窗口下的性能
指标，而不是对未收敛点外推得到的渐近谱半径。

八个程序按四个主题组织：Exp1、Exp2 和 Exp8 给出主比较、两类拓扑的完整路径扫描及
中心问题的插值方法对照；Exp3--5 检验选择质量、稳健性、代价和停止策略；Exp6 考察
固定物理系数场的嵌套加密；Exp7 给出三层 V-cycle 初步结果。

## 3. 两网格主结果与逐步扫描（Exp1、Exp2、Exp8）

### 3.1 网格尺度轴

本表固定对比度为 $10^4$、拓扑为 cross-channel，并按固定粗网格分辨率组织两个细网格
序列。表中每个性能项均写为“循环数 / 有效收敛因子”。

| 固定粗网格 | $1/h$ | $1/H$ | adaptive $m$ | adaptive | global-reference |
|---|---:|---:|---:|---:|---:|
| $1/H=8$ | 32 | 8 | 4 | 37 / 0.682669 | 54 / 0.772358 |
| $1/H=8$ | 64 | 8 | 8 | 123 / 0.893149 | 264 / 0.948901 |
| $1/H=16$ | 64 | 16 | 21 | 65 / 0.806080 | 321 / 0.957779 |
| **$1/H=16$（中心）** | **128** | **16** | **43** | **242 / 0.944407** | **3227 / 0.995727** |
| $1/H=16$ | 256 | 16 | 85 | 998 / 0.986251 | 13948 / 0.999010 |

尺度表仅改变 cross-channel 问题的网格。另一个 256/16 大规模 winding-ring 扩展问题上，
adaptive 与 global-reference 分别为 1002 / 0.986299 和 3950 / 0.996508。
...
| **128, 16** | **$10^4$（中心）** | **43** | **242 / 0.944407** | **3227 / 0.995727** |
...
|---|---|---:|---:|---:|
| **128, 16** | **cross（中心）** | **43** | **242 / 0.944407** | **3227 / 0.995727** |
...
### 3.4 几何插值与能量极小插值的中心问题对照

Exp8 固定中心问题，即 128/16 网格、对比度 $10^4$、cross-channel、seed 1 和常数右端
项。两种方法共用矩阵、粗点、Galerkin 构造、一次前向与一次后向 Gauss--Seidel、零初值
及 $10^{-6}$ 相对残量标准，仅更换插值矩阵。每种方法进行一次确定性求解，不设置循环数
上限，并持续迭代至达到停止标准。

| 插值 | 构造 | 循环数 | 最终相对残量 | $\rho_{\mathrm{eff}}$ | $P$ 密度 | $J(P)$ | $A_c$ 非零元数 |
|---|---|---:|---:|---:|---:|---:|---:|
| 几何插值 | 双线性 | 85524 | $9.99896215\times10^{-7}$ | 0.999838472 | 1.395003% | $2.40960908\times10^{10}$ | 1849 |
| 能量极小插值 | 列相对残量 $10^{-10}$ | 3227 | $9.97423567\times10^{-7}$ | 0.995727131 | 97.841982% | $8.41927184\times10^9$ | 50625 |

能量极小插值将循环数降至几何插值的 $1/26.503$，说明它在该高对比中心问题上已经是
显著优于标准几何插值的基准。主结果中的有限步 adaptive 进一步把循环数由 3227 降至
242；这项额外收益来自路径位置选择，而不是因为能量极小端点本身薄弱。能量极小插值的
改善同时伴随更高的插值与粗算子密度，表中一并给出以完整呈现性能与存储代价。

### 3.5 两类拓扑的完整路径扫描

Exp2 固定 128/16 和对比度 $10^4$，在 cross-channel 与 winding-ring 上对
$m=1,\ldots,128$ 的每个整数步数分别计算 $J(W_m)$ 与 $\rho_{\mathrm{eff}}(m)$。上面板
绘制归一化能量差

$$
\widehat J_m=
\frac{J(W_m)-J(W_{\mathrm{ref}})}
     {J(W_1)-J(W_{\mathrm{ref}})},
$$

下面板绘制实际两网格残量历程得到的 $\rho_{\mathrm{eff}}$。两份 CSV 均保留全部 128
个点及原始能量。

![cross-channel 完整路径](code/results/experiment2_cross_channel_path.png)

![winding-ring 完整路径](code/results/experiment2_winding_ring_path.png)

| 拓扑 | 扫描区间内最小因子位置 | 最小 $\rho_{\mathrm{eff}}$ | adaptive $m$ / $\rho_{\mathrm{eff}}$ | reference $\rho_{\mathrm{eff}}$ | $\max_m[J(W_{m+1})-J(W_m)]$ |
|---|---:|---:|---:|---:|---:|
| cross-channel | 38 | 0.9417637 | 43 / 0.9444069 | 0.9957271 | $-2.9954302\times10^1$ |
| winding-ring | 53 | 0.9410121 | 43 / 0.9431693 | 0.9821588 | $-1.1524200\times10^{-2}$ |

两条路径的最大相邻能量增量均为负，故 $J(W_m)$ 在整个扫描区间内保持下降；相应的
$\rho_{\mathrm{eff}}(m)$ 则先降后升并伴随局部波动。这里的 $m=38$ 和 $m=53$ 均只指
$m=1,\ldots,128$ 扫描区间内的最小值，不作区间外最优性声明。
...
中心 128/16 问题采用相同机器内、双方预热后各五次、交替顺序测量。双方均以几何插值
为初值并使用同一 Jacobi--PCG 路径；reference 继续推进至各列相对残量 $10^{-10}$。
表中仅报告五次正式测量的算术平均时间。

| 方法 | setup/ms | solve/ms | total/ms | 循环数 / $\rho_{\mathrm{eff}}$ |
|---|---:|---:|---:|---:|
| adaptive | 391.970 | 229.785 | 621.755 | 242 / 0.944407 |
| global-reference | 2264.647 | 7489.791 | 9754.438 | 3227 / 0.995727 |

在该环境中，global-reference 的平均总时间是 adaptive 的 15.69 倍。时间结果用于同机
端到端比较；循环数和 $\rho_{\mathrm{eff}}$ 提供不依赖计时波动的补充指标。
...
v8.4 的数值证据支持一个集中结论：在当前高对比通道问题、固定粗点和指定磨光器下，
沿能量插值路径选择有限步位置可以同时降低构造成本和两网格循环数。两类拓扑的完整
路径扫描表明，插值能量单调下降并不要求实际有效收敛因子同步下降；主比较、seed/RHS、
停止消融和多层初试说明这一优势并非由单一算例或单一右端项产生。中心问题对照进一步
确认，能量极小插值相对几何插值本身已有显著优势，而有限步选择在这个强基准上继续
改善。固定物理场加密中优势明显缩小，因而本文最合适的数值定位是面向网格尺度通道
结构的高对比求解策略，并把固定物理加密作为边界与补充证据。
