...
这里 $m_{\mathrm{opt}}$ 表示搜索窗内的连续路径最优点。固定候选数的离散损失由归一化
覆盖半径控制；当前 reuse 集合在 $[1/8,1/2]$ 上的连续覆盖半径为 $1/12$。
...
相同时取较小 $m$。若 pilot 残量或收缩率非有限、或者 $\widehat\rho_m\notin(0,1)$，
则把预测值记为正式循环上限；所有预测均在该上限截断。令
$e_m=\widehat N_m-N_m$，$m_{\mathcal M}$ 为候选集内真实
...
收敛的方法；计时入口仅在三者均达到正式容差后生成表格，并报告预热后五次测量的
Q1、中位数和 Q3。
...
有限 pilot 预测仍是策略的主要误差来源。设计组与验证组分开报告，以保持留出检验的
独立性。
...
|---|---:|---:|---:|---:|
| adaptive-fast | 364.110（341.662--372.527） | 212.904（203.262--224.411） | 567.372（557.049--585.431） | 242 |
| adaptive-reuse | 863.756（854.391--865.292） | 209.242（209.230--216.588） | 1072.997（1061.645--1097.731） | 242 |
| global-reference | 1874.376（1842.051--1878.422） | 7017.694（6947.565--7255.852） | 9014.608（8821.942--9134.274） | 3227 |
